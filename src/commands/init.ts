...
import {buildCredentialStoreRef, getDefaultCredentialStoreType, saveAccessToken,} from "../credentials/store.js";
...
export async function runInit(args: string[]): Promise<void> {
  ...
  const isFirstInit = !existingConfig;

  const config: ConfigFile = {
    version: 1,
    defaultProfile: isFirstInit || setDefault ? profileName : existingConfig.defaultProfile,
    profiles: {
